namespace Sparrow.ConvertSystem.Test
{
    internal partial class SparrowConvertTest
    {
        ...
        public void SparrowConvertChineseNumberTest(float dec, string simple, string traditional)
        {
            Assert.Multiple(() =>
            {
        ...
        public void SparrowConvertChineseNumberTest(short dec, string simple, string traditional)
        {
            Assert.Multiple(() =>
            {
                Assert.That(SparrowConvert.ToChineseNumber(dec, ChineseCategory.Simple), Is.EqualTo(simple));
        ...
        public void SparrowConvertChineseNumberTest(int dec, string simple, string traditional)
        {
            Assert.Multiple(() =>
            {
                Assert.That(SparrowConvert.ToChineseNumber(dec, ChineseCategory.Simple), Is.EqualTo(simple));
        ...
        public void SparrowConvertChineseNumberTest(long dec, string simple, string traditional)
        {
            Assert.Multiple(() =>
            {
                Assert.That(SparrowConvert.ToChineseNumber(dec, ChineseCategory.Simple), Is.EqualTo(simple));
        ...
        [TestCase("111987654321", "一千一百一十九亿八千七百六十五万四千三百二十一", "壹仟壹佰壹拾玖亿捌仟柒佰陆拾伍萬肆仟叁佰贰拾壹")]
        [TestCase("1111987654321", "一兆一千一百一十九亿八千七百六十五万四千三百二十一", "壹兆壹仟壹佰壹拾玖亿捌仟柒佰陆拾伍萬肆仟叁佰贰拾壹")]
        public void SparrowConvertChineseNumberTest(string dec, string simple, string traditional)
        {
            Assert.Multiple(() =>
            {
                Assert.That(SparrowConvert.ToChineseNumber(dec, ChineseCategory.Simple), Is.EqualTo(simple));
