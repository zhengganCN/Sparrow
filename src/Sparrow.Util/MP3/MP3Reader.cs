    ...
    public class MP3Reader
    {
        ...
        public MP3_ID3V1 GetMP3_ID3V1(string path, Encoding encoding)
        {
            ...
            if (Encoding.UTF8.GetString(id3v1.Take(3).ToArray()) == "TAG")
            {
                ...
                {
                    Title = GetDecodedData(id3v1.Skip(3).Take(30).ToArray(), encoding),
                    Artist = GetDecodedData(id3v1.Skip(33).Take(30).ToArray(), encoding),
                    Album = GetDecodedData(id3v1.Skip(63).Take(30).ToArray(), encoding),
                    Year = GetDecodedData(id3v1.Skip(93).Take(4).ToArray(), encoding),
                    Comment = GetDecodedData(id3v1.Skip(97).Take(28).ToArray(), encoding),
                    Reserve = GetDecodedData(id3v1.Skip(125).Take(1).ToArray(), encoding),
                    Track = GetDecodedData(id3v1.Skip(126).Take(1).ToArray(), encoding),
                    Genre = GetDecodedData(id3v1.Skip(127).Take(1).ToArray(), encoding)
                };
            }
            else
            {
        ...
        private string GetDecodedData(byte[] dataBytes, Encoding encoding)
        {
            return encoding.GetString(dataBytes).TrimEnd('\0');
        }
