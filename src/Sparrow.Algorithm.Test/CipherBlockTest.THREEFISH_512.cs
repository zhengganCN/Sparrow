...
namespace Sparrow.Algorithm.Test
{
    public partial class CipherBlockTest
    {
        ...
        public void THREEFISH_512(CipherMode mode, CipherPadding padding)
        {
            var cipher = new SparrowBlockCipher();
            cipher.SetCipher(CipherBlockAlgorithm.THREEFISH_512, mode, padding);
            var encrypted = cipher.Encrypt(CipherConst.Original512Bytes, CipherConst.Key512Bytes);
            var decrypted = cipher.Decrypt(encrypted, CipherConst.Key512Bytes);
            var data = Encoding.UTF8.GetString(decrypted);
            Assert.That(CipherConst.Original512 == data, Is.True);
        }
