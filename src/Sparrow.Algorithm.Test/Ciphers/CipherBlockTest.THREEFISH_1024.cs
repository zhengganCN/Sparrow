...
namespace Sparrow.Algorithm.Test
{
    public partial class CipherBlockTest
    {
        ...
        public void THREEFISH_1024(CipherMode mode, CipherPadding padding)
        {
            var cipher = new SparrowBlockCipher();
            cipher.SetCipher(CipherBlockAlgorithm.THREEFISH_1024, mode, padding);
            var encrypted = cipher.Encrypt(CipherConst.Original1024Bytes, CipherConst.Key1024Bytes);
            var decrypted = cipher.Decrypt(encrypted, CipherConst.Key1024Bytes);
            var data = Encoding.UTF8.GetString(decrypted);
            Assert.That(CipherConst.Original1024Bit == data, Is.True);
        }
